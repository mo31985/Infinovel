...
import React, { useState, useEffect, useCallback, useRef, Fragment } from 'react'; // Import Fragment
import { initializeApp } from 'firebase/app';
import {
  getAuth, signInAnonymously, onAuthStateChanged,
  createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut,
  GoogleAuthProvider, signInWithPopup, signInWithCustomToken // 引入 Google 登入相關模組
} from 'firebase/auth';
...
// Helper function to sanitize strings for JSX rendering
const sanitizeForJSX = (str) => {
  if (typeof str !== 'string') {
    return str === null || str === undefined ? '' : String(str);
  }
  // Escape only basic HTML entities. Let React handle { and } when rendered as children directly.
  let sanitized = str.replace(/&/g, '&amp;') // Escape & first
                   .replace(/</g, '&lt;')   // Escape <
                   .replace(/>/g, '&gt;')   // Escape >
                   .replace(/"/g, '&quot;') // Escape "
                   .replace(/'/g, '&#039;');  // Escape '
...
// Centralized message display hook
const useMessages = () => {
  const [message, setMessage] = useState('');
  const [messageType, setMessageType] = useState('info'); // 'info', 'success', 'error'
  const messageTimeoutRef = useRef(null);

  const displayMessage = useCallback((msg, type = 'info', duration = 3000) => {
    if (messageTimeoutRef.current) {
      clearTimeout(messageTimeoutRef.current);
    }
    // Sanitize message *before* setting the state
    setMessage(sanitizeForJSX(msg));
    setMessageType(type);
    messageTimeoutRef.current = setTimeout(() => {
      setMessage('');
      setMessageType('info'); // Reset to info after timeout
    }, duration);
  }, []);
...
function App() {
  ...
  // Consolidated messages into one state using useMessages hook
  const { message: globalMessage, messageType: globalMessageType, displayMessage: showGlobalMessage } = useMessages();
  ...
  const loadProgress = useCallback(async (isManualLoad = false) => {
    if (!db || !auth || !auth.currentUser) {
      showGlobalMessage('無法載入進度：未登入。', 'error');
      return false;
    }
    setIsLoadingSave(true);
    showGlobalMessage('正在載入進度...', 'info');
    ...
    try {
      ...
      if (isManualLoad && currentLoadCount >= currentCalculatedLoadLimit) {
        showGlobalMessage(`您已達到載入次數上限 (${currentCalculatedLoadLimit} 次)。`, 'error');
        return false;
      }
      ...
      if (docSnap.exists()) {
        ...
        if (data.currentChapterState) {
          ...
          showGlobalMessage('進度載入成功！', 'success');
          ...
          showGlobalMessage('沒有找到有效的保存進度。', 'info');
          return false;
        }
      } else {
        showGlobalMessage('沒有找到保存的進度。', 'info');
        return false;
      }
    } catch (err) {
      console.error("載入進度時出錯:", err);
      showGlobalMessage(`載入進度失敗: ${err.message}`, 'error');
      return false;
    } finally {
      setIsLoadingSave(false);
    }
  }, [db, auth, userProfile, showGlobalMessage]);
  ...
  const saveProgress = useCallback(async (isAutoSave = false) => { // 新增 isAutoSave 參數
    if (!db || !auth || !auth.currentUser || !currentChapter) { // 確保 currentChapter 存在
      ...
      showGlobalMessage('無法保存進度：未登入或無當前章節數據。', 'error');
      return;
    }

    if (!isAutoSave) { // 只在手動保存時更新 UI 狀態
      setIsSaving(true);
      showGlobalMessage('正在保存進度...', 'info');
    }
    ...
    try {
      ...
      if (!isAutoSave && currentSaveCount >= currentCalculatedSaveLimit) { // 只在手動保存時檢查限制
        showGlobalMessage(`您已達到保存次數上限 (${currentCalculatedSaveLimit} 次)。`, 'error');
        return;
      }
      ...
      if (!isAutoSave) { // 只在手動保存時更新計數
        ...
        showGlobalMessage('進度保存成功！', 'success');
      } else {
        console.log('自動保存成功。');
      }
    } catch (err) {
      ...
      if (!isAutoSave) {
        showGlobalMessage(`保存進度失敗: ${err.message}`, 'error');
      }
    } finally {
      if (!isAutoSave) { // 只在手動保存時更新 UI 狀態
        setIsSaving(false);
  ...
  }, [db, auth, currentChapter, characterStats, userProfile, userPathHistory, showGlobalMessage]);
  ...
  const loadOrInitStory = useCallback(async () => {
    ...
    setIsLoadingSave(true);
    showGlobalMessage('正在載入您的進度...', 'info');
    ...
    try {
      if (db && auth && auth.currentUser && userId !== '載入中...') {
        ...
        if (!loaded) {
          ...
          showGlobalMessage('請創建您的偵探！', 'info');
        } else {
        ...
        showGlobalMessage('無法載入保存進度，請先登入或以訪客身份開始。', 'info');
      }
    } catch (err) {
      console.error("在初始故事載入或初始化期間發生錯誤:", err);
      showGlobalMessage(`應用程式初始設定失敗: ${err.message}.`, 'error');
  ...
  }, [db, auth, userId, loadProgress, showGlobalMessage]);

  // 調用 LLM 生成下一章節內容
  const generateNextChapter = useCallback(async (userChoiceText, selectedChoiceId, isAutoChoice = false) => {
    setIsLoadingText(true);
    showGlobalMessage('AI 正在生成下一章節文本...', 'info', 60000); // 延長顯示時間，因為生成可能較慢
    setChoices([]);
    ...
    if (db && auth && auth.currentUser && currentChapter && selectedChoiceId) {
      ...
      try {
        ...
        // Do not set global error for stats update as it shouldn't stop story generation
      }
    }
    ...
    try {
      ...
      if (textResult.candidates && textResult.candidates.length > 0 &&
        ...
        showGlobalMessage('LLM 文本回應格式不正確或內容缺失。', 'error');
        setCurrentChapter({
          title: '生成失敗',
      ...
      showGlobalMessage(`生成章節時發生錯誤: ${err.message}. 請檢查控制台獲取更多詳細信息。`, 'error');
      setCurrentChapter({
        title: '生成失敗',
  ...
  }, [db, auth, currentChapter, characterStats, getCurrentChapterUniqueId, saveProgress, showGlobalMessage]); // 將 showGlobalMessage 加入依賴
        ...
        setTimeLeft(prevTime => {
          if (prevTime <= 1) {
            ...
            if (defaultChoice) {
              ...
              showGlobalMessage('時間用盡，但沒有找到無限制的預設選項！故事可能卡關。', 'error');
              setCurrentChapter({
                title: '故事中止',
  ...
  }, [currentChapter, generateNextChapter, showGlobalMessage]);
  ...
  const handleConfirmStats = () => {
    if (remainingPoints === 0) {
      ...
      showGlobalMessage(`請分配所有 ${INITIAL_STAT_POINTS} 點。剩餘點數：${remainingPoints}`, 'error');
    }
  };
  ...
  const handleFeedbackSubmit = async () => {
    if (!db || !auth || !auth.currentUser) {
      showGlobalMessage('請登入以提交回饋。', 'info');
      return;
    }
    if (!feedbackText.trim()) {
      showGlobalMessage('回饋內容不能為空。', 'info');
      return;
    }

    showGlobalMessage('正在提交回饋...', 'info');
    ...
    try {
      await addDoc(feedbackCollectionRef, {
        userId: userUid,
        feedbackText: feedbackText, // Store raw text, sanitize only for display
        chapterId: currentChapter ? currentChapter.chapterId : 'initial_load',
        timestamp: Date.now(),
      });
      showGlobalMessage('回饋提交成功！感謝您的寶貴意見。', 'success');
    } catch (err) {
      console.error("提交回饋時出錯:", err);
      showGlobalMessage(`提交回饋失敗: ${err.message}`, 'error');
    }
  };
  ...
  const handleRestartGame = useCallback(async () => {
    if (!db || !auth || !auth.currentUser) {
      showGlobalMessage('無法重新開始遊戲：未登入。', 'error');
      return;
    }
    ...
    try {
      ...
      setChoices([]);
      setIsLoadingText(false);
      showGlobalMessage('', 'info', 0); // Clear messages immediately
      setIsSaving(false);
      setIsLoadingSave(false);
      setChapterChoiceStats({});
      ...
      setShowProceedToChoicesButton(false);
      setFeedbackText('');
      showGlobalMessage('遊戲已重新開始，所有進度已清除！', 'success');

      setShowRestartConfirm(false);

    } catch (err) {
      console.error("重新開始遊戲時出錯:", err);
      showGlobalMessage(`重新開始遊戲失敗: ${err.message}`, 'error');
      setShowRestartConfirm(false);
    } finally {
      setLoading(false);
    }
  ...
  const handleAuthSubmit = async () => {
    ...
    if (!auth) {
      showGlobalMessage('Firebase 認證服務未準備好，無法執行登入/註冊。', 'error');
    ...
    try {
      if (authMode === 'register') {
        ...
        showGlobalMessage('註冊成功！您已自動登入。', 'success');
      } else { // login
      ...
      let rawErrorMessagePart = err.message || '';
      ...
      if (err.code === 'auth/email-already-in-use') {
        ...
        const projectId = FIREBASE_CONFIG.projectId || 'N/A';
        errorMessage = `此認證操作未啟用。請前往 Firebase Console (專案 ID: ${projectId}) 並在「建置」->「身份驗證」->「登入方式」中，啟用「電子郵件/密碼」和「匿名」登入方式。`;
      } else {
        errorMessage = `認證失敗: ${rawErrorMessagePart}`;
      }
      showGlobalMessage(errorMessage, 'error', 5000);
    }
  };
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      showGlobalMessage('您已成功登出。', 'success');
      ...
      showGlobalMessage(`登出失敗: ${err.message}`, 'error');
    }
  };
  ...
  const handleStartAsGuest = async () => {
    ...
    if (!auth) {
      showGlobalMessage('Firebase 認證服務未準備好。', 'error');
      console.warn('Guest login attempted but auth is not initialized.');
      return;
    }
    showGlobalMessage('正在以訪客身份登入...', 'info');
    console.log('Attempting signInAnonymously...');
    try {
      await signInAnonymously(auth);
      showGlobalMessage('以訪客身份登入成功！', 'success');
      ...
      showGlobalMessage(`以訪客身份登入失敗: ${err.message}.`, 'error');
    }
  };
  ...
  useEffect(() => {
    ...
    const initFirebase = async () => {
      try {
        if (!FIREBASE_CONFIG || Object.keys(FIREBASE_CONFIG).length === 0 || !FIREBASE_CONFIG.apiKey || !FIREBASE_CONFIG.authDomain || !FIREBASE_CONFIG.projectId) {
          ...
          showGlobalMessage(`錯誤! ${configErrorMsg}`, 'error');
          setUserId(null);
          setIsAuthReady(true);
          setLoading(false);
          setShowCharacterCreation(false);
        ...
        const unsubscribe = onAuthStateChanged(authInstance, async (user) => {
          console.log('onAuthStateChanged triggered. User object:', user ? user.uid : 'null (no user)');
          ...
          if (user) {
            currentUid = user.uid;
            isRegisteredUser = !user.isAnonymous;
            userEmailAddress = user.email;
            setUserId(currentUid);

            console.log(`onAuthStateChanged: User state: UID=${currentUid}, Registered=${isRegisteredUser}, Email=${userEmailAddress}`);

            const userProfileRef = doc(firestoreInstance, `artifacts/${APP_ID}/users/${currentUid}/profile`, 'user_data');
            try {
              ...
              if (profileSnap.exists()) {
                ...
                setUserProfile(prev => ({
                  ...prev,
                  loadCount: profileData.loadCount || 0,
                  saveCount: profileData.saveCount || 0,
                  isVIP: profileData.isVIP || false,
                  isRegistered: isRegisteredUser,
                  email: userEmailAddress
                }));
                console.log("onAuthStateChanged: User profile loaded:", profileData);
              } else {
                console.log("onAuthStateChanged: User profile not found, creating new one.");
                ...
                console.log("onAuthStateChanged: New user profile created.");
              }
            } catch (profileError) {
              console.error("onAuthStateChanged: Error fetching/creating user profile:", profileError);
              showGlobalMessage(`載入/創建用戶資料失敗: ${profileError.message}. 保存/載入功能可能受限。`, 'error');
            }
            // 關鍵：一旦用戶認證成功，隱藏歡迎畫面並準備觸發故事載入
            console.log('onAuthStateChanged: User authenticated. Setting showWelcomeScreen to false and initialDataLoaded to false.');
            setShowWelcomeScreen(false);
            setInitialDataLoaded(false); // 重置此狀態以確保 loadOrInitStory 被觸發
          } else {
            // 沒有用戶登入，確保顯示歡迎畫面並重置用戶相關狀態
            console.log('onAuthStateChanged: No user authenticated. Setting userId to null, resetting userProfile, and showing welcome screen.');
            setUserId(null);
            setUserProfile({ loadCount: 0, saveCount: 0, isVIP: false, isRegistered: false, email: null });
            setShowWelcomeScreen(true);
          }
          setIsAuthReady(true);
          setLoading(false); // Firebase auth state checked, hide global loading
          console.log(`onAuthStateChanged: isAuthReady set to true, loading set to false.`);
        });
        ...
        console.error("Firebase 初始化失敗 (useEffect catch block):", error);
        showGlobalMessage(`Firebase 初始化失敗: ${error.message}. 請檢查控制台獲取更多詳細信息。`, 'error');
        setIsAuthReady(true);
        setUserId(null);
        setLoading(false);
        setShowCharacterCreation(false);
    ...
    initFirebase();
  }, [showGlobalMessage]); // 空依賴陣列表示這只會在組件掛載時運行一次
  ...
  useEffect(() => {
    const fetchChoiceStats = async () => {
      ...
      try {
        ...
        showGlobalMessage(`獲取選擇統計數據失敗: ${err.message}`, 'error');
        setChapterChoiceStats({});
      }
    };
  ...
  }, [db, currentChapter, getCurrentChapterUniqueId, isAuthReady, showGlobalMessage]);

  // Helper function to render percentage message (extracted from JSX)
  const getPercentageMessage = (percentage) => {
    if (percentage === 'N/A') return ''; // Return empty string if no message
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black flex flex-col items-center justify-center p-4 sm:p-6 relative">
        <div className="w-full max-w-md bg-gray-800 shadow-xl rounded-xl p-6 sm:p-8 text-center border border-purple-700">
          ...
          {globalMessage && globalMessageType === 'error' && (
            <div className="bg-red-800 text-red-200 px-4 py-3 rounded relative mb-4 border border-red-600" role="alert">
              <strong className="font-bold">錯誤!</strong>
              <span className="block sm:inline" dangerouslySetInnerHTML={{ __html: globalMessage }}></span>
            </div>
          )}
          {globalMessage && globalMessageType !== 'error' && (
             <div className={`mb-4 p-3 rounded-lg text-center font-semibold
               ${globalMessageType === 'success' ? 'bg-green-700 text-green-100' : 'bg-blue-700 text-blue-100'}`}
               dangerouslySetInnerHTML={{ __html: globalMessage }}>
             </div>
           )}
        ...
        {showAuthModal && (
          <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50"> {/* Increased z-index to z-50 */}
            <div className="bg-gray-800 rounded-xl shadow-2xl p-8 max-w-md text-center border border-purple-700 animate-fade-in-up">
              ...
              {globalMessage && globalMessageType === 'error' && (
                <div className="bg-red-700 text-red-100 px-4 py-2 rounded-lg mb-4" dangerouslySetInnerHTML={{ __html: globalMessage }}></div>
              )}
              <input
                type="email"
                placeholder="電子郵件"
  ...
  return (
    ...
    <Fragment>
      <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black text-gray-100 p-4 sm:p-6 flex flex-col items-center">
          ...
          <div className="flex items-center space-x-2">
            {userProfile.isRegistered ? (
              <span className="font-semibold text-white">
                已登入：<span className="text-blue-300 break-all">{sanitizeForJSX(userProfile.email || userId)}</span>
              </span>
            ) : (
              <span className="font-semibold text-white">
                匿名用戶：<span className="font-mono text-blue-300 break-all">{sanitizeForJSX(userId)}</span>
              </span>
            )}
        ...
        <main className="w-full max-w-4xl bg-gray-800 shadow-xl rounded-xl p-6 mb-8 flex-grow border border-purple-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {choices.map((choice, index) => {
                  ...
                  return (
                    ...
                    >
                      <span>
                        {sanitizeForJSX(choice.text)}
                        {requirementAndGainText && <span className="ml-2 text-sm opacity-80">{sanitizeForJSX(requirementAndGainText)}</span>}
                      </span>
                    </button>
                  );
                })}
          ...
          {!isLoadingText && choices.length === 0 && currentChapter && (
            <div className="mt-8 pt-6 border-t-2 border-gray-700 bg-gray-800 rounded-xl p-6 text-center text-xl text-gray-400 font-inter">
              ...
              {userPathHistory.map((pathItem, index) => (
                <div key={index} className="bg-gray-700 p-4 rounded-lg shadow-md border border-gray-600">
                  <p className="text-gray-300 text-sm font-semibold mb-1">
                    章節 {index + 1}: {sanitizeForJSX(pathItem.chapterId)}
                  </p>
                  <p className="text-white text-lg">
                    你選擇了: <span className="font-bold text-teal-300">"{sanitizeForJSX(pathItem.choiceText)}"</span>
                  </p>
                  {pathItem.percentage && pathItem.percentage !== 'N/A' && (
                    <p className="text-gray-400 text-sm">
                      <span className="font-semibold">{sanitizeForJSX(pathItem.percentage)}%</span> 的玩家也選擇了這條路徑。
                      <span className={`${parseFloat(pathItem.percentage) < 30 ? 'text-orange-400' : parseFloat(pathItem.percentage) > 70 ? 'text-blue-400' : 'text-green-400'}`} dangerouslySetInnerHTML={{ __html: getPercentageMessage(pathItem.percentage) }}>
                      </span>
                    </p>
                  )}
                </div>
      ...
      {/* 登入/註冊彈窗 - CORRECTLY PLACED INSIDE THE MAIN RETURN'S WRAPPER DIV */}
      {showAuthModal && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 p-8 rounded-lg shadow-xl w-full max-w-md">
            ...
            {globalMessage && globalMessageType === 'error' && (
              <p className="text-red-500 text-center mb-4 font-inter" dangerouslySetInnerHTML={{ __html: globalMessage }}></p>
            )}
            {globalMessage && globalMessageType !== 'error' && (
              <p className={`text-center mb-4 font-inter ${globalMessageType === 'success' ? 'text-green-400' : 'text-blue-400'}`} dangerouslySetInnerHTML={{ __html: globalMessage }}></p>
            )}
